<# 
.SYNOPSIS 
    This script displays is a functional script used to run basic troubleshooting steps for slow internet services
.DESCRIPTION 
    This script uses standard Windows command line tools and speedtest.net to determine internet speeds and response times. The tools include:
    1. WMI to get workstation CPU and Memory usage as a percent
    2. Test-NetConnection - ping (ICMP) provides an understanding on reaction time of your connection I.E. how fast you get a response after you’ve sent out a request.
    3. tracert is used for displaying the route (path) and measuring transit delays of packets across a network.
.NOTES 
    File Name  : internet-speedtest.ps1 
    Author     : Ryan Sclanders 
    Requires   : Windows PowerShell V2
#> 

# Lets start by getting the date
$date = get-date -uformat "%d%m%Y"

# We will use the users Documents Directory to add the resultant log file
$user = $env:username
$LogFile = "C:\Users\$user\Documents\Internet-Speedtest-$date.log"


# Adding title information to the top of the log file



# Now we include Hardware information and the current untilization average as a percent
